...
public class CurveChartView extends View {
    ...
    public void setShowModel(int model) {
        ...
        if (model == MODEL_WEEK)
            for (int i = 0; i < 7; i++) {
                pointX.add((float) ((i + 0.5) * averageWidth));
            }
        else
            for (int i = 0; i < 12; i++)
                pointX.add((float) ((i + 0.5) * averageWidth));
     ...
     * @param pointY1
     */
    public void setPointsY(final List<Float> pointY1) {
        if (null != pointY1) {
            if(!haveFocuse)
    ...
    private void setYDatas(final List<Float> pointY1) {
        pointY = pointY1;
    ...
    public float transformYCoordinate(float y) {
        ...
        return viewHeigth - ((y / 100f) * viewHeigth);
    }
