...
public class MyPieChartRenderer extends AbstractChartRenderer {
    ...
    private static final float DEFAULT_LABEL_LINE_RADIUS_FACTOR = 1.1f;
    private static final float DEFAULT_LABEL_LINE_RADIUS_FACTOR2 = 1.2f;
    ...
    private char[] labelBuffer2 = new char[64];
    private int labelMarginLeftOrRight = 10;
    private int labelMarginLeftOrRight2 = labelMarginLeftOrRight + 30;
    private int labelMarginTopOrBottom = 5;
    ...
    public MyPieChartRenderer(Context context, Chart chart, PieChartDataProvider dataProvider) {
        super(context, chart);
        ...
        linePaint.setColor(Color.parseColor("#D33A31"));
        ...
        centerCircleText1Paint.setTextAlign(Paint.Align.CENTER);

        centerCircleText2Paint.setAntiAlias(true);
        centerCircleText2Paint.setTextAlign(Paint.Align.CENTER);
    ...
    private void drawLabel(Canvas canvas, SliceValue sliceValue, float lastAngle, float angle) {
        ...
        final int numChars2 = valueFormatter.formatChartValue2(labelBuffer2, sliceValue);
        ...
        final float labelWidth2 = labelPaint.measureText(labelBuffer2, labelBuffer2.length - numChars2, numChars2);
        ...
        if (hasLabelsOutside) {
            float label1StartX;
            float label2StartX;
            final float labelLineRadius;
            if (Math.abs(rawX - centerX) < circleRadius / 3){
                labelLineRadius = circleRadius * DEFAULT_LABEL_LINE_RADIUS_FACTOR2;
            } else {
                labelLineRadius = circleRadius * DEFAULT_LABEL_LINE_RADIUS_FACTOR;
            }
            float lineX1 = labelLineRadius * sliceVector.x + centerX;
            float lineY1 = labelLineRadius * sliceVector.y + centerY;
            float lineX2;
            if (rawX > centerX) {
                // Right half.
                lineX2 = lineX1 + labelWidth * 1.5f;
                label1StartX = lineX2 - labelMarginLeftOrRight - labelWidth;
                label2StartX = lineX2 -labelWidth2 - labelMarginLeftOrRight2;
            } else {
                lineX2 = lineX1 - labelWidth * 1.5f;
                label1StartX = lineX2 + labelMarginLeftOrRight;
                label2StartX = lineX2 + labelMarginLeftOrRight2;
            }
            top = lineY1 - labelMarginTopOrBottom;
            bottom = lineY1 + labelHeight + labelMarginTopOrBottom;
            // 画指示线
            linePaint.setColor(sliceValue.getDarkenColor());
            circlePaint.setColor(sliceValue.getDarkenColor());
            canvas.drawLine(rawX, rawY, lineX1 , lineY1, linePaint);
            canvas.drawLine(lineX1, lineY1, lineX2 , lineY1, linePaint);
            canvas.drawCircle(lineX2, lineY1, smallCircleRadius, circlePaint);
            canvas.drawText(labelBuffer, labelBuffer.length - numChars, numChars, label1StartX, top, labelPaint);
            canvas.drawText(labelBuffer2, labelBuffer2.length - numChars2, numChars2, label2StartX, bottom, labelPaint);
        }
